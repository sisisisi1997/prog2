...
main (int argc, char *argv[])
{
	...
	kiFile << "Eredeti fa másolás előtt:" << std::endl;
	...
	kiFile << std::endl << "Másolat kiegészítés előtt:" << std::endl;
	kiFile << binFa2;
	...
	binFa2 << '0' << '1' << '0' << '1' << '1' << '1' << '1' << '1' << '1' << '1';
	
	kiFile << std::endl << "Másolat kiegészítés után:" << std::endl;
	kiFile << binFa2;
	...
	kiFile << std::endl << "Eredeti fa másolat kiegészítése után:" << std::endl;
	kiFile << binFa;		// ehhez kell a globális operator<< túlterhelése, lásd fentebb
    // (jó ez az OO, mert mi ugye nem igazán erre gondoltunk, amikor írtuk, mégis megy, hurrá)
